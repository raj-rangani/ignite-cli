    ...
    if [[ "${framework}" == "nodejs" || "${framework}" == "mern" || "${framework}" == "mean" ]]; then
        {
            ...
            echo "LOG_LEVEL=${is_production:+info}${is_production:-debug}"mework}" == "angular" ]]; then
        ...
        case "${role}" in
            ...
            mobile)
                ...
                case "${framework_choice}" in
                    1)
                        framework="flutter"
                        # If Flutter is selected, create a Flutter project right away
...
function create_flutter_project() {
    ...
    # First check if we have previously saved values from flutter.sh command
    ...
    return 0
} # Flutter project creation function for new version
