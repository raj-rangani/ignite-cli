...
inquirer.default.registerPrompt("autocomplete", require("inquirer-autocomplete-prompt"));
...
export async function confirm(args: {
  ...
  if (!args.nonInteractive && !args.force) {
    ...
    throw new IgniteError("Pass the --force flag to use this command in non-interactive mode");
  } else {
    return true;
  }
}
...
export async function promptForDirectory(args: {
  ...
  while (!dir) {
    ...
    if (args.relativeTo) {
      ...
      target = path.resolve(promptPath);
    }
