...
const DEFAULT_CHILDREN: NonNullable<IgniteErrorOptions["children"]> = [];
const DEFAULT_EXIT: NonNullable<IgniteErrorOptions["exit"]> = 1;
const DEFAULT_STATUS: NonNullable<IgniteErrorOptions["status"]> = 500;
...
export class IgniteError extends Error {
  ...
  readonly name = "IgniteError";
  readonly original: Error | undefined;
  readonly status: number;

  constructor(message: string, options: IgniteErrorOptions = {}) {
    super();
...
/**
 * Checks if a IgniteError is caused by attempting something
